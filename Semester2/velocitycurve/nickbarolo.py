# -*- coding: utf-8 -*-
"""
Created on Thu Feb 15 17:45:43 2024
...
########################### use variables ###################################
SaveFigName = 'C:/Users/christopher/onedrive/documents/4th_year_physics/mphys_project/s2/w1-3/barolo/k=7_Barolo_curve.png'
SaveFig = True
...
def plot_initial(rad, vrot, vdisp, A, n, A2,n_u):
    ...
    ax.errorbar(rad, vrot, yerr= vdisp * doi/2, fmt='o', color='blue', label='Rotation Velocity')
...
def plot_curve(rad, vrot, vdisp, rad1, vrot1, vdisp1, A_scaled, A1_scaled, A2_scaled, n, n_err):
    ...
    ax.errorbar(rad/au, 10**(-3) * vrot, yerr= 10**(-3) * vdisp * doi, fmt= 'o', color='indigo')
    ax.errorbar(rad1/au, 10**(-3) * vrot1, yerr= 10**(-3) * vdisp1 * doi, fmt= 'o', color ='gold', label='Observed data')
    #ax.plot(rad1, vrot1, 'o', color ='gold', label='Observed data')
    ax.plot(rad_fine/au, 10**(-3) * model(rad_fine, A_scaled), '-', color='indigo', label = f'Model Data\nM = {SMass:.2f} $\pm$ 2.35 M$_\odot$')
    ax.plot(rad_fine/au, 10**(-3) * non_keplarian_model(rad_fine, A1_scaled, n), '-', color='green', label = f'Free Fit\nn = {n:.3f} $\pm$ {n_err:.3f}')
    ...
    ax.set_ylabel('Rotation Velocity (km/s)')
    ax.set_title('K=7 Rotation Curve')
    ax.grid(alpha=0.3)
