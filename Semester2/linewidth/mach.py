# -*- coding: utf-8 -*-
"""
Created on Tue Apr 16 17:07:06 2024
...
LINE = 'C:/Users/Christopher/onedrive/documents/4th_year_physics/mphys_project/S2/W7-9/linewidth/Linewidth_XCLASS.fits'
THERMAL = 'C:/Users/Christopher/onedrive/documents/4th_year_physics/mphys_project/S2/W7-9/linewidth/Average_Thermal_linewidth.fits'
...
def main():
    ...
    therm = fits.open(THERMAL)
    line = fits.open(LINE)
    red = fits.open(RED)
    continuum = fits.open(CONTIN)
    
    line[0].data = match_shapes_with_nan(line[0].data)
    
    new_data = line[0].data/therm[0].data
    mask = (new_data > 8)
    new_data[mask] = 8
    print(np.nanmean(new_data))
    plotting(new_data, continuum, red, therm)
    
    return
    
def plotting(line_map, continuum, red, hdul):
    ...
    plt.contour(xx_c,yy_c,continuum_2d, levels = 25, alpha = 0.2, cmap='copper',
    ...
    plt.title('Mach Number', size=13)
    name = OUT + 'Mach_number.pdf'
